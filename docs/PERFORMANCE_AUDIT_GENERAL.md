...
Дата: 2026-07-20 · Версия: `0.20.1` · Метод: чтение исходников `src/` (~60 файлов, ссылки `file:line` по рабочему дереву), прод-сборка для замера бандла.
...
> **Статус P0 (2026-07-20): реализовано** — находки 1, 2 и 5 из таблицы ниже закрыты:
> composited-гибрид зума в проде; авто-canvas для статических рендереров при
> ≥500 элементов (`CANVAS_PREVIEW_ELEMENT_THRESHOLD`) + новый `CanvasStaticPreview`;
> rAF-коалесцинг коммитов контролов в `FieldControls` (draft-mirror + синхронный
> flush на commit-жестах). Тесты: обновлены `tests/animationPreview.test.ts`
> (порог canvas) и `e2e/geometry-authority.production.spec.ts` (гейты геометрии
> пинят `svg-dom`, т.к. читают bounds из SVG). Следующий шаг — P1 (волновое поле
> в воркер, градиент-растр SDF, halftone-градиент, `contain: layout paint`).

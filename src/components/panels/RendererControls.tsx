...
function Range({ label, value, min, max, hardMax, step = 1, description, onChange }: { label: string; value: number; min: number; max: number; hardMax?: number; step?: number; description?: string; onChange: (value: number) => void }) {
 ...
 return <NumericRange parameter={`renderer.${label.toLowerCase().replaceAll(" ", "-").replaceAll("/", "-")}`} label={label} value={value} min={min} max={max} hardMax={hardMax} step={step} resetValue={defaults[label] ?? value} description={description} onChange={onChange} />;
}
